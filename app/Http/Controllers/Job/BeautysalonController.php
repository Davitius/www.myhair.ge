...
class BeautysalonController extends Controller
{
    public function index()
    {
        ...
        if (isset($salon)) {
            $staffs = User::where('sal_id', $salon->id)->orderBy('saladdtime', 'DESC')->paginate(10);
    ...
    public function search(StaffSearchRequest $request)
    {
        ...
        $salon = Salon::where('user_id', Auth::user()->id)->first();
        $staffs = User::where('sal_id', $salon->id)->orderBy('id')->paginate(10);
        $s_staffs = User::where('staffcode', $search['staffcode'])->get();
