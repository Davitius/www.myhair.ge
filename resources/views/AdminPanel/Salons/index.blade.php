            ...
            <div class="row mb-2">
                ...
                <div class="col-sm-6">
                    <ol class="breadcrumb float-sm-right">
                        ...
                        <li class="breadcrumb-item">სალონები</li>
                    </ol>
                </div><!-- /.col -->
            </div><!-- /.row -->
        </div><!-- /.container-fluid -->
                ...
                <div class="col-12 mt-3">
                    <div class="card">
                        ...
                        <div class="card-header">
                            ...
                            <form method="get" action="{{route('SalonSearch')}}">
                                <div class="card-tools">
                                    <div class="btn-group">
                                        <input type="text" name="SalonSearch" id="SalonSearch" class="form-control"
                                               placeholder="სალონის დასახელება">
                            ...
                            @if(isset($Salons))
                                @if(count($Salons))
                                    <table class="table table-hover text-nowrap">
                                        ...
                                        @foreach($Salons as $Salon)
                                            ...
                                            <tr>
                                                <td>{{$Salon->id}}</td>
                                                <td>{{$Salon->name}}</td>
                                                <td @if($Salon->status == "Active") style="color: green"
                                                    @else style="color: red" @endif >{{$Salon->status}}</td>
                                                <td>{{$Salon->created_at}}</td>
                                                <td>{{$Salon->updated_at}}</td>
                                                <td>
                                                    <a href="{{ route('AdminSalons.edit', $Salon->id) }}">
                                    ...
                                    <div class="PaginBtn mt-3 d-flex justify-content-center">
                                        <label
                                            class="">{{ $Salons->appends(['StaffSearch' => request()->SalonSearch])->links() }}</label>
                                    </div>
                                @else
                                    <div
